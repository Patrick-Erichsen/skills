...
change kind: runtime | micro | ui | docs | other
value/proof:
...
Assign one qualified PR and one exact isolated worktree, using `gwt` when installed or native `git worktree` otherwise.
...
- Add proportional proof: focused regression tests for behavior, browser or visual proof for UI, and source/link/command validation for docs.
