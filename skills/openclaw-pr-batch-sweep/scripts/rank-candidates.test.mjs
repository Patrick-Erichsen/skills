...
test("keeps a one-line production patch when the remaining gates pass", () => {
  const output = runHydrated(
    hydratedPr({
      files: [
...
test("keeps a tiny diagnostic wording change when the remaining gates pass", () => {
  const output = runHydrated(
    hydratedPr({
...
test("keeps mechanical micro-fixes when the remaining gates pass", () => {
  const output = runHydrated(
    hydratedPr({
...
test("does not reject a lifecycle micro-fix based on size alone", () => {
  const output = runHydrated(
    hydratedPr({
...
test("keeps focused UI work", () => {
  const output = runHydrated(
    hydratedPr({
      title: "fix(tui): keep selected row visible while scrolling",
      files: [
...
test("keeps docs-only changes", () => {
  const output = runHydrated(
    hydratedPr({
      title: "docs(memory): correct CPU-only VPS guidance",
      changed_files: 1,
      files: [{ filename: "docs/memory/vps.md", additions: 4, deletions: 2 }],
      additions: 4,
      deletions: 2,
    }),
  );
  ...
  assert.equal(output.rejected.length, 0);
  assert.equal(output.selected.length, 1);
  assert.equal(output.selected[0].docsChange, true);
});
...
test("excludes a carried PR only while its exact head is unchanged", () => {
  const directory = mkdtempSync(path.join(tmpdir(), "openclaw-pr-ledger-"));
  const ledgerPath = path.join(directory, "ledger.json");
  writeFileSync(
    ledgerPath,
    JSON.stringify({
      carried: [{ number: 12345, headSha: "abcdef1234567" }],
    }),
  );
