...
const LOW_SIGNAL_TITLE =
  /^(?:test|chore|refactor|ci|build|infra)(?:\([^)]*\))?:|^(?:fix|chore)\((?:test|ci|build|infra)\):|\b(lint|coverage|unit tests?|add tests?)\b/i;
...
if (decisionLedgerPath) {
  ...
  const carried = ledger.carried ?? [];
  if (!Array.isArray(carried)) {
    throw new Error("Decision ledger field carried must be an array");
  }
  for (const entry of carried) {
    if (typeof entry !== "object" || entry === null) {
      throw new Error("Carried ledger entries must include number and headSha");
    }
    const number = parsePrNumber(String(entry.number ?? entry.ref ?? entry.url ?? ""));
    const headSha = String(entry.headSha ?? entry.head_sha ?? "").trim();
    if (number === null || !/^[0-9a-f]{7,40}$/i.test(headSha)) {
...
function analyze(pr) {
  ...
  const testFiles = files.filter((file) => TEST_PATH.test(filePath(file)));
  const docsChange =
    paths.some((path) => DOC_PATH.test(path)) &&
    paths.every((path) => DOC_PATH.test(path) || TEST_PATH.test(path));
  ...
  const currentHead = String(
    pr.headRefOid ?? pr.head_sha ?? pr.head?.sha ?? "",
  ).toLowerCase();
