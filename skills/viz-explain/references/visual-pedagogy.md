...
GitHub Pages project sites add the repository name to the URL. For this repo,
placing the tree at `.well-known/agent-skills/` in the built site produces:
...
That scoped path works with the current Vercel provider when the installer is
given `https://patrick-erichsen.github.io/skills`, because it probes a
path-relative `.well-known` index first. It is not the origin-root URL described
by the draft RFC. If strict origin-root discovery becomes important, serve the
same files from a custom domain or the account's root Pages site without the
`/skills` prefix.
...
| **8. Event timeline/history** | What happened when across a real or hypothetical interval? Use a shared time axis, evidence markers, and uncertainty; unlike a request sequence, events need not form one causal call chain. UML timing and interaction semantics provide reusable notation ([UML 2.5.1, Clause 17](https://www.omg.org/spec/UML/2.5.1/PDF)). | Reconstruct an automation from schedule fire through tool activity, report creation, announcement, and later inspection. |
...
- use **Request/interaction sequence** for one causal scenario and reserve
  **Event timeline/history** for chronology and reconstruction;
